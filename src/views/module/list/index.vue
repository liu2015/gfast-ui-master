...
<script>
import { listList, getList, delList, addList, updateList } from '@/api/module/list'
export default {
  name: "list",
  data() {
    return {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除module编号为"' + sp_ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delList(sp_ids);
        }).then(() => {
          this.getList();
